    ...
    while (!WindowShouldClose())
    {
        ...
        { // Physics Step/*
            ...
            for (auto entity : allAsteroids)
            {
                ...
                {
                    float speed = Vector2Distance(position.position, GetMousePosition()) > 300 ? 0 : Vector2Distance(position.position, GetMousePosition());
                    physics.forces = Vector2Add(physics.forces, Vector2Scale(Vector2Scale(getNormalizedMouseDirectionVector(position.position), speed), 10.0f));
                    physics.acceleration = physics.forces;

                    physics.velocity = Vector2Add(physics.velocity, Vector2Scale(physics.acceleration, TIMESTEP));
                    physics.velocity = Vector2Subtract(physics.velocity, Vector2Scale(physics.velocity, 2 * TIMESTEP));
                ...
                if (physics.attracts == true)
                {
                    ...
                    // physics.velocity.x = physics.velocity.x + (100/ Vector2Distance(position.position, GetMousePosition()));
                    // physics.velocity.y = physics.velocity.y + (100/ Vector2Distance(position.position, GetMousePosition()));
                    position.position = Vector2Add(position.position, Vector2Scale(physics.velocity, TIMESTEP));
                }
                else
                {
